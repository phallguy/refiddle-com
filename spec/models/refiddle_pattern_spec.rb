...
describe RefiddlePattern do
  describe "validations" do
    ...
    # Disabling for now cause the UI really does support regex without enclosing // and it's really
    # hard to check for a valid regix across flavors without actually parsing it.
    #
    # ["/./", "/aa/g", "/k[^s\/]{1,}s*/g"].each do |valid_pattern|
    #   it "allows #{valid_pattern}" do
    #     build( :refiddle_pattern, regex: valid_pattern ).should be_valid
    #   end
    # end

    # ["./", "/aag" ].each do |invalid_pattern|
    #   it "prevents #{invalid_pattern}" do
    #     build( :refiddle_pattern, regex: invalid_pattern ).should_not be_valid
    #   end
    # end
